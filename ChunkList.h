...
namespace fefu_laboratory_two
{
    ...
    template <typename ValueType>
    class Chunk final : ChunkListInterface<ValueType>
    {
    public:
        using reference = ValueType&;
        using difference_type = std::ptrdiff_t;
        ...
        explicit Chunk(const size_type chunk_size) : size(chunk_size)
        {
            list = allocator.allocate(size);
        }

        Chunk(const size_type chunk_size, Allocator<value_type> custom_allocator) : size(chunk_size),
        ...
        ~Chunk() override
        {
            allocator.deallocate(list, size);
        }
        ...
        {
            if (position >= size || static_cast<std::ptrdiff_t>(position) < 0)
            {
    ...
    protected:
        ...
        ChunkListInterface<value_type>* chunk = nullptr;
        int index = 0;

    public:
        ChunkList_iterator() noexcept = default;

        ChunkList_iterator(pointer current_value, size_type current_index,
                           ChunkListInterface<value_type>* current_chunk) :
        ...
        friend void swap(ChunkList_iterator<ValueType>& first, ChunkList_iterator<ValueType>& second) noexcept
        {
        ...
        ChunkList_iterator& operator--()
        {
            ...
            {
                throw std::out_of_range("Decrementing before begin iterator.");
            }
            return *this;
        }
        ...
        {
            ChunkList_iterator result = *this;
            result += difference;
            return result;
        }
        ...
        {
            if (index + difference < chunk->GetSize())
            {
                index += difference;
                value = &chunk->At(index);
            }
    ...
    public:
        ...
        ChunkList_const_iterator(value_type* value, int index, ChunkListInterface<value_type>* chunk) :
        ...
        ChunkList_const_iterator(pointer value, int index, const ChunkListInterface<value_type>* chunk) :
            ChunkList_iterator<value_type>(const_cast<value_type*>(value), index,
                                           const_cast<ChunkListInterface<value_type>*>(chunk))
        {
        }
        ...
        friend void swap(ChunkList_const_iterator<ValueType>& first,
                         ChunkList_const_iterator<ValueType>& second) noexcept
        {
            std::swap(first.value, second.value);
            std::swap(first.chunk, second.chunk);
            std::swap(first.index, second.index);
        ...
        {
            return ChunkList_const_iterator(&this->chunk->At(this->index + difference), this->index + difference,
                                            this->chunk);
        }
    ...
    class ChunkList final : ChunkListInterface<T>
    {
    public:
        using value_type = T;
    ...
    public:
        ...
        explicit ChunkList(const size_type count, const T& value = T(), const Allocator& alloc = Allocator()) : start(
            nullptr)
        {
            ...
            try
            {
                for (size_type i = 0; i < count; ++i)
                {
                    if (i % N == 0)
                    {
                        ...
                        {
                            start = temp_pointer = new Chunk<value_type>(N, alloc);
                        }
                    }
                    temp_pointer->list[i % N] = value;
        ...
        reference At(const size_type pos) override
        {
            ...
            const size_type chunk_number = pos / N;
            size_type value_number = pos % N;
            Chunk<value_type>* temp_pointer = start;
        ...
        reference operator[](const difference_type pos) override
        {
            const size_type chunk_number = static_cast<size_type>(pos) / N;
            size_type value_number = static_cast<size_type>(pos) % N;
            Chunk<value_type>* temp_pointer = start;
            for (size_type i = 0; i < chunk_number; i++)
            {
                temp_pointer = temp_pointer->next;
        ...
        const_reference operator[](const difference_type pos) const
        {
            const size_type chunk_number = static_cast<size_type>(pos) / N;
            size_type value_number = static_cast<size_type>(pos) % N;
            Chunk<value_type>* temp_pointer = start;
        ...
        reference front()
        {
            if (size > 0)
                return start->list[0];
            throw std::out_of_range("ChunkList is empty!");
        }
        ...
        {
            return ChunkList_const_iterator<value_type>(&At(size), size, this);
        }
        ...
        {
            const size_type value_number = size % N;
        ...
        void clear() noexcept
        {
            ...
            {
                const Chunk<value_type>* temp_pointer = current_chunk;
        ...
        iterator erase(const_iterator first, const_iterator last)
        {
            ...
            const int end_index = last.GetIndex();

            auto iterator = begin() + start_index;
            for (int i = end_index; i < size; ++i)
            {
                *iterator = std::move(*(iterator + (end_index - start_index)));
                ++iterator;
            }
            ...
            while (size < max_size() && temp_pointer->current_size > 0)
            {
                --temp_pointer->current_size;
                --size;
            }
        ...
        void pop_back()
        {
            ...
            temp_pointer->current_size--;
            size--;
            if (temp_pointer->current_size == 0 && temp_pointer->prev != nullptr)
            {
        ...
        {
            std::swap(other.start, this->start);
